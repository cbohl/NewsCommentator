...
**Version**: 1.6.1 | **Ratified**: 2026-02-19 | **Last Amended**: 2026-02-27
...
- **1.6.1 (2026-02-27)**: Simplified tool assignments to one specialized tool per persona. Historian (Maggie) gets Wikipedia only, Economist (Tim) gets Yahoo Finance only, Philosopher (Sofia) gets Tavily only. Removed shared Tavily access from historian and economist to force use of specialized tools. Rationale: with shared web search available, personas defaulted to Tavily instead of using their specialized tools; restricting to one tool per persona produces more distinctive, expertise-aligned research. No backwards-compatibility impact — existing footnotes with "(Web)" labels remain valid.
